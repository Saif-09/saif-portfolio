...
- [[D016 - WebGL Uses Plain Three Not R3F]] · [[D017 - Defer External Service Activation]] · [[D018 - Multi-page and Header Nav]] · [[D019 - Visual Richness Overhaul]] · [[D020 - Art Direction v2 Bold Monochrome]] · [[D021 - Slate and Vouch as Featured Case Studies]] · [[D022 - Ziyarex OreoNotch and Clickk on the Site]]
...
- **OreoNotch price mismatch**, `lib/products.ts` says `$12`, the live site and checkout say **$12.99** ([[D022 - Ziyarex OreoNotch and Clickk on the Site]]). The manifest feeds the store, OG images, the public API and the strip on nine subdomains, so every surface but the checkout is wrong.
- **Clickk case study?**, currently a grid card. Its in-page keyboard demo and the soundpack licensing work would carry a full study if it earns one.
- **Vouch visual**, currently a screenshot of its GitHub README ([[D021 - Slate and Vouch as Featured Case Studies]]). A capture of the real localhost dashboard would be stronger, and needs a repo whose data is safe to publish, so not the Shoppin' one.
