...
| | Cold | Warm | Needs |
|---|---|---|---|
| GitHub Actions (fallback, works today) | ~90s | ~90s | nothing |
| This service | ~2-4s | ~300ms | a Cloud Run deploy |
...
packages `resume.tex` actually loads, so it is small enough that Cloud Run can
cold-start it in a few seconds and serve a warm request in a few hundred
milliseconds.
